...
export function calcMovingAvg(dataPoints, window) {
    ...
    let fastMA = movingAvg(5)
    let slowMA = movingAvg(10)
...
// utility sleep to avoid rate limits
const sleep = (ms) => new Promise(res => setTimeout(res, ms))

export async function calcMeanReversion(symbol, start, end, interval) {

    // fetch target security prices
    const { data: targetData } = await axios.get(
        `http://localhost:5001/api/security`,
        { params: { symbol, start, end, interval } }
    )
    const dataPoints = targetData.map(d => ({ date: d.date, price: d.close }))

    // fetch recommended symbols
    const { data: recommendedSymbols } = await axios.get(
        `http://localhost:5001/api/security/recommendation`,
        { params: { symbol } }
    )
    ...
    for (let rec of recommendedSymbols) {
        console.log(rec)
        // avoid hitting Yahoo too fast
        await sleep(2000)

        const { data } = await axios.get(
            `http://localhost:5001/api/security`,
            { params: { symbol: rec, start, end, interval } }
        )

        const recDataPoints = data.map(d => ({ date: d.date, price: d.close }))
        recommendedSecs.push({ symbol: rec, dataPoints: recDataPoints })
    }

    // find best correlated security
    let bestCorrelated = { symbol: "", dataPoints: [], correlation: -1 }
    const targetPrices = dataPoints.map(d => d.price)
